...
type AddStepWithoutData<T extends Step> = {
  step: T | string | Promise<{ default: T }>;
  tasks?: T['tasks'];
  options?: Partial<StepOptions>;
};

type AddStepWithData<T extends Step> = AddStepWithoutData<T> & {
  data: T['data'];
};

export type AddStep<T extends Step = any> = T['data'] extends undefined | void
...
export default class StepHandler<SD = any, TD = any, TR = any> {
   ...
   * @static
   */
  static async create(data: CreateStep): Promise<StepHandler> {
    const id = generate();
    ...
    // Create step modules
    const { module, path, filename } = await StepHandler.getModule(data.module);
    const name = slugify(data.name || filename);

    if (!data.moduleFn) {
      if (typeof module.beforeAll === 'function') {
        await StepHandler.create({
          ...data,
          module,
    ...
    const stepInstance = new StepHandler({
      id,
      key,
      name,
      index: data.index,
   ...
   * @param key
   */
  public static async getByKey(key: string): Promise<StepHandler> {
    ...
    return new Promise((resolve, reject) => {
      deFlow.client.get(key, (err, res) => {
        ...
        const step = new StepHandler(stepJSON);
        return resolve(step);
      });
    });
  }
  ...
  static async nextTask(stepKey: string): Promise<void> {
    ...
    const step = await StepHandler.getByKey(stepKey);
   ...
   */
  public async addAfter<T extends Step>(params: AddStep<T>): Promise<StepHandler> {
    const { options, tasks, step: addStep } = params;

    const step = await StepHandler.getModule(addStep);
   ...
   */
  public async getPrevious(): Promise<StepHandler | undefined> {
    debug('getPrevious');
    ...
    return new Promise((resolve) => {
      ...
      this.#deflow.client.send_command(
        ...
        (err, reply: string[]) => {
          ...
          return resolve(new StepHandler(json));
        }
      );
    });
  }
  ...
  static async getModule(
    path: string | Step | Promise<any>
  ): Promise<{ path: string; module: Step; filename: string }> {
    try {
      let module: Step | undefined;
   ...
   */
  async #getModule(): Promise<{ module: Step; path: string }> {
    return StepHandler.getModule(this.#module);
  }
   ...
   */
  async #getHandlerStep(): Promise<StepHandler> {
    if (this.#parentKey) {
      return StepHandler.getByKey(this.#parentKey);
    } else {
      return this;
    }
  }
  ...
  async #runOnHandlerError(task: Task, error: Error): Promise<void> {
    const { module } = await StepHandler.getModule(this.#module);
